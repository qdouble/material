...
export class CustomValidators {
  static compare(c1Key: string, c2Key: string, errorMsg: string): ValidatorFn {
    return (group: FormGroup): { [key: string]: any } => {
      ...
      let error = {};
      error[errorMsg] = true;
      return c1.value !== c2.value ? error : <any>null;
    };
  }
...
export function requiredIfGroup(
  mainControl: string,
  ...
  return (group: FormGroup): { [key: string]: any } => {
    ...
    for (let control of otherControls) {
      if (group.get(mainControl).value) {
        if (
          isBlank(group.get(control).value) ||
          (typeof group.get(control).value === 'string' && group.get(control).value == '')
        ) {
          requiredErr = true;
          break;
        }
    ...
    return requiredErr ? { [errMessage]: true } : <any>null;
  };
}

export function requiredIfCondition(
  condition: boolean,
  otherControls: string[],
  errMessage: string
): ValidatorFn {
