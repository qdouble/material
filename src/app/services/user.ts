...
import {
  ChangeSelectedPrizeResponse,
  CheckIfUserUpdatedAtResponse,
  CheckIPMatchResponse,
  CheckReferrerUsernameResponse,
  DismissProfileChangesResponse,
...
export class UserService extends RequestBase {
  ...
  changeSelectedPrize(id: string) {
    return this.http.get<ChangeSelectedPrizeResponse>(
      `${API_USER_URL}/changeSelectedPrize?id=${id}`,
      this.optionsNoPre
    );
  }
  ...
  checkIfUserUpdated() {
    return this.http.get<CheckIfUserUpdatedAtResponse>(
      `${API_USER_URL}/checkIfUserUpdated`,
      this.optionsNoPre
    );
  }
  ...
  checkIPMatch(sponsor: string) {
    return this.http.get<CheckIPMatchResponse>(
      `${API_USER_URL}/checkIPMatch?referredBy=${sponsor}`,
      this.optionsNoPre
    );
  }
  ...
  checkReferrerUsername(username: string) {
    return this.http.get<CheckReferrerUsernameResponse>(
      `${API_USER_URL}/checkReferrerUsername?username=${username}`,
      this.optionsNoPre
    );
  }
  ...
  dismissProfileChanges() {
    return this.http.get<DismissProfileChangesResponse>(
      `${API_USER_URL}/dismissProfileChanges`,
      this.optionsNoPre
    );
  }
  ...
  getProfile() {
    return this.http.get<GetProfileResponse>(`${API_USER_URL}/getProfile`, this.optionsNoPre);
  }

  getReferral(id: string) {
    return this.http.get<GetReferralResponse>(
      `${API_USER_URL}/getReferral?id=${id}`,
      this.optionsNoPre
    );
  }

  loginAdmin(user: User) {
    return this.http.post<GenericResponse>(`${API_ADMIN_URL}/login`, user, this.options);
  }

  loginUser(user: User) {
    return this.http.post<GenericResponse>(`${API_USER_URL}/login`, user, this.options);
  }

  logout() {
    return this.http.get(`${API_USER_URL}/logout`, { ...this.optionsNoPre, responseType: 'text' });
  }
  ...
  registerUser(user: User) {
    return this.http.post<GenericResponse>(`${API_USER_URL}/registerUser`, user, this.options);
  }

  removeReferrals(ids: string[]) {
    return this.http.post<RemoveReferralsResponse>(
      `${API_USER_URL}/removeReferrals`,
      ids,
      this.options
    );
  }
  ...
  resetPassword(reset: { email: string; code: string; password: string }) {
    return this.http.post<GenericResponse>(`${API_USER_URL}/resetPassword`, reset, this.options);
  }

  setSponsor(sponsorUsername: string) {
    return this.http.post<SetSponsorResponse>(
      `${API_USER_URL}/setSponsor`,
  ...
  updateProfile(user: User) {
    return this.http.post<GetProfileResponse>(`${API_USER_URL}/updateProfile`, user, this.options);
  }
}
